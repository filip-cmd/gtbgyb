  ...
  <body>
  	<header>
		...
		<div class="clanci">
			...
			<hr>
			<?php
			$query = "SELECT * FROM clanak WHERE arhiva=0 AND kategorija='IT' LIMIT 3";
			 $result = mysqli_query($dbc, $query);
			 $i=0; 
			 while($row = mysqli_fetch_array($result))
			  	{
			...
			<hr>
			<?php
			$query = "SELECT * FROM clanak WHERE arhiva=0 AND kategorija='Zabava' LIMIT 3";
			 $result = mysqli_query($dbc, $query);
			 while($row = mysqli_fetch_array($result))
			  	{
			  	 echo '<article>';
